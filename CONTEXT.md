...
- **觸價待確認 (restore confirmation)**: 觸價執行端(重新)啟動或切回觸價單建立時的環境後,第一筆行情就已穿過觸發價的停損／停利／括號單出場;不自動送單,由使用者選擇送出、取消或保留。保護超過 60 秒未運作(串流連線且伺服器模式已知時,沒有任何行情或心跳;含斷線、靜默停滯、休眠、模式未知)後也比照;60 秒以內不算。價格提醒不適用。
