...
- **觸價待確認 (restore confirmation)**: 觸價執行端(重新)啟動或切回觸價單建立時的環境後,第一筆行情就已穿過觸發價的停損／停利／括號單出場;不自動送單,由使用者選擇送出、取消或保留。保護連續超過 60 秒無法運作(串流未連線或伺服器模式未知)後恢復,或名義上連線中卻超過 90 秒沒有任何行情或心跳(靜默停滯、休眠)也比照;60 秒以內的中斷不算。價格提醒不適用。
- **重新穿價 (re-cross)**: 「保留」後的觸價單須先看到價格回到觸發價另一側,再次穿過才會觸發。
