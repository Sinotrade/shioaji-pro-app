...
- **確認條件**：同帳戶讀到的列，order.id 與帳戶都相同，狀態為 Cancelled，而且累計 `cancel_quantity` ≥ 刪單前
  剩餘量（原始 `order.quantity` 減掉開始時的已成交量）。`status.order_quantity` 不使用，因為 1.7.6 HTTP 列會回 0。
  回讀列的已成交量比本地少時，不算證據。
...
- **投影**：確認後的列帶 `confirmed` 旗標發布。trading-state 接受它，即使期間已收到其他回報（通常就是 Cancel
  ...
  維持「改刪待確認」。
- **額度**：同帳戶同時刪多筆（閃電全刪）時共用進行中的讀取，但只加入在本筆刪單送出之後才開始的讀取。

## 限制

- Shioaji#234（模擬減量後刪單，HTTP 與 SSE 不一致）上游仍開啟。1.7.6 模擬的減量後刪單回讀是累計取消 2、
  Cancelled，這只是回歸證據，不代表正式環境的行為。
- 成交與刪單同時發生時，取消量會小於刪單前的剩餘量，結果判為未確認。這是刻意偏保守，需要人工對帳。
