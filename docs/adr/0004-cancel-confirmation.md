...
- **串流 STALE**：#128 的心跳 watchdog 新增 `stale` 狀態。cache 可信度沿用 `tradeCacheContinuous()`（需要 `live`），
  所以 `stale` 與 `down` 一樣不讀 cache，直接做那一次 `refresh:true`；小視窗沿用主視窗的判定。
- **與改單回報確認（mutation-intent）互動**：已確認的取消是終態，會解除同一筆委託先前留下的「改刪待確認」
  （`mutation:<id>`）與等待中的改單意圖，不影響其他委託。
