...
- **確認條件**：同帳戶讀到的列，order.id 與帳戶都相同，狀態為 Cancelled，而且累計 `cancel_quantity` 涵蓋刪單前
  剩餘量（原始 `order.quantity` 減掉開始時的已成交量）。刪單途中若有成交，要取消的量會跟著變少，所以實際判斷是
  「回讀的取消量＋成交量 ≥ 原始量」，也就是沒有剩餘；成交沒有和刪單競爭時，這和前一句的條件完全相同。
  `status.order_quantity` 不使用，因為 1.7.6 HTTP 列會回 0。回讀列的已成交量比本地少時，不算證據。
...
- **投影**：確認後的列帶 `confirmed` 旗標發布。trading-state 接受它，即使期間已收到其他回報（通常就是 Cancel
  ...
  維持「改刪待確認」。
- **額度**：同帳戶同時刪多筆（閃電全刪、委託批次刪單改為並行送出）時共用讀取，但只用本筆刪單送出之後才開始的
  讀取。另一筆刪單的 `refresh:true` 若在本筆送出後才開始，本筆可以免費拿來核對；核對不到，才花自己那唯一一次。
...
- 小視窗沒有委託基準，#128 的前置流程會讓小視窗每筆期貨改刪單先做一次 `refresh:true`；刪單確認另外最多一次，
  而且可與同帳戶其他刪單共用。在小視窗大量全刪時仍可能接近帳務額度，需要原生實測觀察。
