        ...
        <div className={styles.shell}>
            <div className={styles.card}>
                ...
                <div className={styles.fieldGroup}>
                    ...
                    <div className={styles.modeRow}>
                        <button
                            className={
                                styles.modeBtn[
                                    !settings.production ? 'sim' : 'normal'
                                ]
                            }
                            disabled={busy}
                        ...
                        <button
                            className={
                                styles.modeBtn[
                                    settings.production ? 'prod' : 'normal'
                                ]
                            }
                            disabled={busy}
                    ...
                    {settings.production && (
                        <span className={styles.prodWarn}>
                            正式環境下單動用真實資金，且需要 Sinopac.pfx 憑證
                        </span>
                    )}
                </div>
                        ...
                        {settings.caPath && (
                            <div className={styles.inputRow}>
                                <input
                                    ...
                                    disabled={busy}
                                    onChange={(e) =>
                                        patch({ caPasswd: e.target.value })
                                    }
                                />
                                <button
                                    className={styles.eyeBtn}
                                    type='button'
                                    onClick={() => setShowCaPw((v) => !v)}
                                >
                                    {showCaPw ? (
                                        <EyeOff size={14} />
                                    ) : (
                ...
                {busy ? (
                    <>
                        ...
                        <span className={headerStyles.progressTrack}>
                            <span className={headerStyles.progressGlider} />
                        </span>
                    </>
                ) : (
                ...
                <span className={styles.hint}>
                    ...
                    Key？請至永豐 API 管理頁申請。稍後仍可從右上角「伺服器」面板調整。
                </span>
            </div>
        </div>
    );
